...
class RecurringCashflow:
    def __init__(self):
        ...
        self.year_start = 0
        self.year_end = 0
        self.taxable = False  # Specified if this impacts your AGI - Salary and 401k, for example, are both 'taxable',
        # in that the 401k expense is negative taxable income

    def define_monthly(self, monthly_flow, name, monthly_nw=0, yearly_apr=1.0, year_start=0, year_end=sys.maxsize,
                       taxable=False):
        self.define_yearly(monthly_flow * 12, name, monthly_nw * 12, yearly_apr, year_start, year_end, taxable)
        return self

    def define_yearly(self, yearly_flow, name, yearly_nw=0, yearly_apr=1.0, year_start=0, year_end=sys.maxsize,
                      taxable=False):
    ...
    def cashflow(self, year):
        if self.year_start <= year <= self.year_end:
    ...
    def gross_income(self, year):
        if self.year_start <= year <= self.year_end and self.yearly_flow > 0:
            years_accumulated = year - self.year_start
            return self.yearly_flow * (self.yearly_apr ** years_accumulated)
        else:
            return 0
    ...
    def taxable_amount(self, year):
        if self.year_start <= year <= self.year_end and self.taxable:
            years_accumulated = year - self.year_start
            return self.yearly_flow * (self.yearly_apr ** years_accumulated)
        else:
            return 0
...
class CashflowModel:
    def __init__(self, name: str = 'Unnamed', state_tax=None):
        self.cashflows = []
        self.name = name
        self.state_tax = state_tax if state_tax is not None else []
    ...
    def plot_annual_flow(self, year=0):
        ...
        cashflow_labels = [x.name + '(${})'.format(int(y)) if y != 0 else '' for x, y in
                           zip(self.cashflows, cashflow_list)]
    ...
    def plot_monthly_flow(self, year=0):
        ...
        cashflow_labels = [x.name + '(${})'.format(int(y / 12)) if y != 0 else '' for x, y in
                           zip(self.cashflows, cashflow_list)]
        plt.pie(np.absolute(cashflow_list), labels=cashflow_labels)
        ...
        monthly_nwflow_labels = [x.name + '(${})'.format(int(y)) if y != 0 else '' for x, y in
                                 zip(self.cashflows, monthly_nwflow_list)]
...
class FinancialModel:
    ...
    def sim(self, cashflowmodel, nyears=30, initial_nw=0, nw_apr_avg=1.075494565, nw_apr_stdev=0.189442643, nruns=100):
        sim_run_dfs = []
        df_summary = {}
        df_summary['name'] = ('{}+{} @ {}+/-{} for {} yrs'.format(cashflowmodel.name, initial_nw, round(nw_apr_avg, 3),
                                                                  round(nw_apr_stdev, 3), nyears))
        ...
        for run in np.arange(0, nruns):  # should use >= 100 runs for some statistical interest
            ...
            for current_year_id in np.arange(1, nyears):
                ...
                this_year['NetWorth'] = min(self.max_nw, int(
                    last_year['NetWorth'] + this_year['NetWorthInterest'] + last_year.get('ExplicitNWGrowth',
                                                                                          0) + last_year.get(
                        'ExcessCash', 0)))
        ...
        for current_year_id in np.arange(1, nyears):
            ...
            dict_keys = ['NetWorthInterest', 'NetWorth', 'GrossIncome', 'TaxableIncome', 'Expenses', 'IncomeTax',
                         'ExplicitNWGrowth', 'ExcessCash', 'nw_apr', 'Year ID']
            for dk in dict_keys:
    ...
    def plot(self, df_tuple, suffix=':STDEV', autoscale_y=True, max_thousands=5 * 1000, year_start=1, year_end=20):
        ...
        for field in fields_to_plot:
            ...
            for df, df_summary in df_tuple_list:
                top_bottom_differences = [t - b for t, b in
                                          zip(df[field + suffix + ':top'], df[field + suffix + ':bottom'])]
                should_plot_error_bars = True if np.max(top_bottom_differences) > 1000 else False
                if should_plot_error_bars:
                    thisplot = ax.plot(df['Year ID'] + self.year, df[field] / 1000, '-',
                                       label='{}'.format(df_summary['name']))
